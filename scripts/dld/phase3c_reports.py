...
from verify_phase3c import compare_corrective_release
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--package",type=Path,default=DEFAULT_PACKAGE); parser.add_argument("--reports",type=Path,default=DEFAULT_REPORTS); parser.add_argument("--baseline-zip",type=Path,default=DEFAULT_BASELINE)
    ...
    with zipfile.ZipFile(args.baseline_zip.resolve()) as archive:
        old_manifest_hash=hashlib.sha256(archive.read("manifest.json")).hexdigest()
    new_zip=package/"dld-market-phase3c.zip"
    comparison=compare_corrective_release(args.baseline_zip.resolve(),new_zip)
    write(reports/"corrective_release.json",{"reason":"Developer numbers were previously serialized with a zero-only decimal suffix","canonicalization":"positive integral official developer_number to base-10 integer text; scientific notation and unsafe forms rejected","old_zip_sha256":digest(args.baseline_zip.resolve()),"new_zip_sha256":digest(new_zip),"old_manifest_sha256":old_manifest_hash,"new_manifest_sha256":digest(package/"manifest.json"),"new_zip_bytes":new_zip.stat().st_size,"developer_identity":manifest["developer_identity"],"logical_comparison":comparison})
    write(reports/"verification.json",{"package_verified":True,"privacy_scan":"pass","deterministic_generation":"verified by verify_phase3c.py --deterministic","row_count":manifest["total_expected_rows"],"chunk_count":len(manifest["chunks"]),"developer_identity":manifest["developer_identity"],"corrective_comparison":comparison})
