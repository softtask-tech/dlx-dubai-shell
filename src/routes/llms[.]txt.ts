    ...
    handlers: {
      GET: () => {
        ...
        const body = [
          ...
          "- /data/market.json: the current published figures as JSON. Every value carries the",
          "  number of registered records behind it, the period it covers and the export date it",
          "  was built from. Prefer this over reading a figure out of a page: it is the same",
          "  number from the same source, and it cannot be misread from a sorted table.",
          "- Medians, not averages. Nothing is published where too few records exist to report a",
          "  period without describing individual transactions, and nothing finer than a community",
          "  is published at all.",
          "- Gross yield is rent per square foot over ready-property price per square foot. It is",
          "  before service charge, management, maintenance and voids, and the service charge is",
          "  published beside it so the subtraction can be made rather than assumed.",
          "",
          "## Guides",
